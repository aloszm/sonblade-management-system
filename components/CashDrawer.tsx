...
type ViewMode = 'current' | 'history' | 'archives' | 'deleted';
            ...
            <div className="max-w-7xl mx-auto">
                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                    ...
                    <div className="flex gap-2">
                        <button onClick={fetchArchives} className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 text-sm font-medium">
                ...
                {viewMode === 'archives' && (
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-8">
                        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
                            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><Archive className="h-5 w-5 text-sonblade-gold" /> Sesiones Archivadas</h2>
                        </div>
                        {loadingExtra ? (
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {archivedSessions.length === 0 ? (
                                        ...
                                        <tr key={a.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-3 text-gray-500">{a.closed_at ? new Date(a.closed_at).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' }) : '-'}</td>
                                            ...
                                            <td className="px-6 py-3 text-gray-500">{a.archived_by}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
        ...
        <div className="max-w-7xl mx-auto">
            <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                    {viewMode === 'current' ? (
                        <>
                            <button onClick={() => setShowNewMovement(true)} className="px-3 py-2 bg-sonblade-gold text-black rounded-lg text-sm font-bold hover:bg-yellow-500 flex items-center gap-2">
                                <Plus className="h-4 w-4" /> Nuevo Movimiento
                            </button>
                            <button onClick={fetchHistory} className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 text-sm font-medium">
                                <History className="h-4 w-4" /> Historial
                            </button>
                            <button onClick={fetchArchives} className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 text-sm font-medium">
                                <Archive className="h-4 w-4" /> Archivos
                            </button>
                            <button onClick={fetchDeleted} className="px-3 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 text-sm font-medium">
                                <Eye className="h-4 w-4" /> Eliminados
                            </button>
                            <button onClick={() => setShowResetConfirm(true)} className="px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-red-600 hover:bg-red-100 flex items-center gap-2 text-sm font-bold">
                                <RotateCcw className="h-4 w-4" /> Reiniciar
                            </button>
                        </>
                    ) : (
                        <button onClick={() => setViewMode('current')} className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 flex items-center gap-2 text-sm font-medium">
            ...
            {viewMode === 'history' && (
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 bg-gray-50"><h2 className="text-lg font-bold text-gray-900">Historial de Cierres de Caja</h2></div>
                    {loadingExtra ? (
                        <div className="p-12 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto" /></div>
                    ) : (
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {historySessions.length === 0 ? (
                                    <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">Sin historial</td></tr>
                                    ...
                                    <tr key={hs.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-3 text-gray-500">{hs.closed_at ? new Date(hs.closed_at).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' }) : 'Abierta'}</td>
                                        ...
                                        <td className={`px-6 py-3 text-right font-bold ${!hs.difference ? 'text-gray-500' : hs.difference === 0 ? 'text-green-500' : hs.difference < 0 ? 'text-red-500' : 'text-blue-500'}`}>
            ...
            {viewMode === 'archives' && (
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
                        <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><Archive className="h-5 w-5 text-sonblade-gold" /> Sesiones Archivadas (Reiniciadas)</h2>
                    </div>
                    {loadingExtra ? (
                        <div className="p-12 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto" /></div>
                    ) : (
            ...
            {viewMode === 'deleted' && (
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 bg-red-50">
                        <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2"><Trash2 className="h-5 w-5 text-red-500" /> Movimientos Eliminados</h2>
                    </div>
                    {loadingExtra ? (
                        <div className="p-12 text-center"><Loader2 className="h-8 w-8 animate-spin mx-auto" /></div>
                    ) : (
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {deletedMovements.length === 0 ? (
                                    <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-500">Sin registros eliminados</td></tr>
                                ) : deletedMovements.map(d => {
                                    const rd = d.record_data as any;
                                    return (
                                        <tr key={d.id} className="hover:bg-red-50/50">
                                            <td className="px-6 py-3 text-gray-500">{new Date(d.deleted_at).toLocaleString('es-MX')}</td>
                                            ...
                                            <td className="px-6 py-3 text-gray-500 text-xs">{d.reason || '-'}</td>
                                        </tr>
                                    );
                                })}
                            </tbody>
                    ...
                    <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-lg border-l-8 border-sonblade-success overflow-hidden mb-8">
                        <div className="px-6 py-4 border-b border-gray-100 flex flex-wrap justify-between items-center bg-white/50 backdrop-blur-sm">
                            ...
                            <div className="text-right text-sm text-gray-500 mt-2 sm:mt-0">
                                Abierta por: <strong className="text-gray-900">{session.opened_by}</strong>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                                ...
                                <div className="lg:col-span-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                                        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden hover:border-sonblade-primary transition-colors">
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <div className="text-2xl font-bold text-gray-900">$ {session.total_cash.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
                                            </div>
                                        </div>
                                        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden hover:border-purple-500 transition-colors">
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <div className="text-2xl font-bold text-gray-900">$ {session.total_card.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
                                            </div>
                                        </div>
                                        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden hover:border-orange-500 transition-colors">
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <div className="text-2xl font-bold text-gray-900">$ {session.total_transfer.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {pendingMovements.length > 0 && (
                                        <div className="bg-yellow-50 border border-yellow-200 rounded-xl overflow-hidden shadow-sm mb-4">
                                                ...
                                                <tbody className="divide-y divide-yellow-100">
                                                    {pendingMovements.map(m => (
                                                        <tr key={m.id} className="hover:bg-yellow-100/50">
                                                            <td className="px-4 py-2 text-gray-500">{new Date(m.created_at).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })}</td>
                                    ...
                                    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                                        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                                            <h3 className="text-gray-900 font-semibold">Movimientos Confirmados</h3>
                                        </div>
                                        <table className="w-full text-left text-sm">
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                ...
                                                {confirmedMovements?.map((m) => (
                                                    <tr key={m.id}>
                                                        <td className="px-4 py-3 text-gray-500">{new Date(m.created_at).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })}</td>
                                                        <td className="px-4 py-3">
                                                            <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium ${m.type === 'sale' ? 'bg-green-100 text-green-700' : m.type === 'expense' ? 'bg-red-100 text-red-700' : m.type === 'deposit' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>
                                                        ...
                                                        <td className="px-4 py-3 text-center">
                                                            <button onClick={() => setConfirmAction({ type: 'delete_movement', id: m.id })} className="p-1 text-red-400 hover:text-red-600 hover:bg-red-50 rounded">
                ...
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                            <h2 className="font-bold text-gray-900">Nuevo Movimiento</h2>
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {(['expense', 'deposit', 'withdrawal'] as const).map(t => (
                                        <button key={t} onClick={() => setNewMovType(t)} className={`px-3 py-2 rounded-lg text-xs font-bold border ${newMovType === t ? 'bg-black text-sonblade-gold border-sonblade-gold' : 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                            ...
                            <div>
                                ...
                                <input type="text" value={newMovDesc} onChange={e => setNewMovDesc(e.target.value)} placeholder="Ej: Compra de suministros" className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">Monto ($)</label>
                                <input type="number" value={newMovAmount} onChange={e => setNewMovAmount(e.target.value)} placeholder="0.00" className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm" />
                            </div>
                        </div>
                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 rounded-b-2xl">
                            <p className="text-xs text-gray-500 mb-3">¿Confirmar movimiento o mantenerlo pendiente?</p>
                            <div className="flex gap-2">
                ...
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-3xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-2xl">
                            <div>
                                <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2"><Lock className="h-5 w-5 text-red-500" /> CERRAR CAJA</h2>
                                <p className="text-sm text-gray-500 mt-1">Sesión abierta hace {elapsedTime} por {session.opened_by}</p>
                            </div>
                                    ...
                                    <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
                                        ...
                                        <div className="flex-1 w-full">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input className="w-full pl-10 pr-4 py-3 text-3xl font-bold text-gray-900 border-2 border-blue-200 rounded-lg focus:ring-4 focus:ring-blue-100 focus:border-sonblade-primary outline-none transition-all text-right"
                        ...
                        <div className="px-8 py-5 bg-gray-50 border-t border-gray-200 rounded-b-2xl flex justify-between items-center">
                            <button onClick={() => setIsModalOpen(false)} className="px-6 py-2.5 text-gray-600 font-medium hover:text-gray-900">Cancelar</button>
                ...
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 className="font-bold text-gray-900">¿Eliminar este movimiento?</h3>
                                <p className="text-sm text-gray-500">Se registrará en el historial de eliminados.</p>
                            </div>
                        </div>
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setConfirmAction(null)} className="px-4 py-2 text-gray-600 font-medium">Cancelar</button>
                ...
                <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 className="font-bold text-gray-900">¿Reiniciar Caja?</h3>
                                <p className="text-sm text-gray-500">Se cerrará la sesión actual, se archivará un snapshot completo, y la caja quedará lista para una nueva apertura.</p>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setShowResetConfirm(false)} className="px-4 py-2 text-gray-600 font-medium">Cancelar</button>
