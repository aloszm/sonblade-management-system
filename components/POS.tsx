    ...
    return (
        <div className="flex h-[calc(100vh-7rem)] flex-col lg:flex-row gap-6 overflow-hidden">
            <div className="flex-1 flex flex-col gap-6 overflow-hidden">
                {/* 1. Barbero & Cliente row */}
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-shrink-0">
                    <div className="bg-[#141414] p-5 rounded-2xl shadow-xl border border-white/5">
                    ...
                    <div className="bg-[#141414] p-5 rounded-2xl shadow-xl border border-white/5 flex flex-col justify-center relative">
                        <h2 className="text-sm font-bold text-sonblade-gold uppercase tracking-wider mb-3 flex items-center gap-2">
                            <Users className="h-4 w-4" /> Cliente (Opcional)
                        </h2>
                        
                        {selectedClient ? (
                            <div className="flex items-center justify-between p-3 border border-sonblade-gold/30 bg-sonblade-gold/10 rounded-xl">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    type="text"
                                    placeholder="Teléfono, nombre o SB-XXXX..."
                                    value={clientSearch}
                                    onChange={(e) => setClientSearch(e.target.value)}
                                    className="w-full pl-9 pr-4 py-2.5 bg-[#1a1a1a] border border-white/10 rounded-xl focus:ring-1 focus:ring-sonblade-gold focus:border-sonblade-gold outline-none transition-all text-sm font-medium text-white placeholder:text-gray-600"
                                />
                                {clientSearch && filteredClients.length > 0 && (
                                    <div className="absolute top-12 left-0 right-0 bg-[#1a1a1a] border border-white/10 shadow-2xl rounded-xl overflow-hidden z-20 flex flex-col max-h-60 overflow-y-auto">
                                        {filteredClients.map(c => (
                                            <button
                                                key={c.id}
                                                onClick={() => { setSelectedClient(c); setClientSearch(''); }}
                                                className="flex items-center justify-between p-3 text-left hover:bg-white/5 border-b border-white/5 last:border-0"
                                            >
                                                <div>
